...
async function buildWebExport(domain, replId) {
  console.log("Building Expo web app for browser visitors...");
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (code === 0) {
        ...
        // Non-fatal: log the error but don't fail the whole build.
        // The serve.js falls back to the QR landing page when no web build exists.
        console.error(`expo export --platform web exited with code ${code} — web build skipped`);
        resolve();
      }
    });
    proc.on("error", (err) => {
      console.error("Web export spawn error:", err.message, "— web build skipped");
      resolve();
    });
  });
}
